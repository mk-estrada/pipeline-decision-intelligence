...
enriched as (
    select
        ...
        coalesce(nullif(pipeline.close_value_amount, 0), products.sales_price, 0) as estimated_deal_value
...
select 
    *,
    case
        when estimated_deal_value = 0 then 'unknown'
        when estimated_deal_value < 5000 then 'small'
        when estimated_deal_value < 20000 then 'medium'
        else 'large'
    end as estimated_deal_size_band
from enriched
